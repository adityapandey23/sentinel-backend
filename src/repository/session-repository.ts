...
export interface SessionRepository {
  ...
  deleteByIdAndUserId(sessionId: string, userId: string, tx?: DbOrTransaction): Promise<boolean>;
  
  // Delete all sessions for a user except the specified one
  deleteAllExcept(userId: string, exceptSessionId: string, tx?: DbOrTransaction): Promise<number>;
}
